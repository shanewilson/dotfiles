...
(defun ensure-lib-from-git (name url)
  (let ((dir (site-lisp-dir-for name)))
    (unless (site-lisp-library-loadable-p name)
      (message "Checking out %s from git" name)
      (save-excursion
        (shell-command (format "git clone %s %s" url dir) "*site-lisp-git*"))
      (add-to-list 'load-path dir))))
...
(defun ensure-site-lisp-libs ()
  (unless (> emacs-major-version 23)
    ...
    (ensure-lib-from-git 'evil "git://gitorious.org/evil/evil.git"))
